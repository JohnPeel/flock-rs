...
fn calculate_separation(boid: Boid, position: Vec3, boids: &[(Boid, Vec3)], width: f32, height: f32) -> Vec3 {
    ...
    for (other, other_pos) in boids.iter() {
        if boid.id != other.id {
            let difference = position - normalize_pos_to(*other_pos, position, width, height);
...
fn update_flocks(time: Res<Time>, window: Res<WindowDescriptor>, params: Res<Vec<FlockParameters>>, mut query: Query<(&mut Boid, &Translation)>) {
    let averages = calculate_averages(params.clone(),  &mut query);
    
    let width = (window.width / 2) as f32;
    let height = (window.height / 2) as f32;
    for (mut boid, position) in &mut query.iter() {
        let alignment = calculate_alignment(boid.max_speed, averages[boid.flock_id].average_forward);
        let cohesion = calculate_cohesion(position.0, averages[boid.flock_id].average_position, params[boid.flock_id].flock_radius);
        let separation = calculate_separation(*boid, position.0, &averages[boid.flock_id].boids, width, height);
...
fn movement_system(time: Res<Time>, window: Res<WindowDescriptor>, mut query: Query<(&Boid, &mut Rotation, &mut Translation)>) {
    for (boid, mut heading, mut position) in &mut query.iter() {
        ...
        position.0 = normalize_pos_to(position.0, Vec3::zero(), (window.width / 2) as f32, (window.height / 2) as f32);
...
impl Plugin for FlockingPlugin {
    fn build(&self, app: &mut AppBuilder) {
        app
            ...
            .add_system_to_stage("movement", movement_system.system());
    }
}
